...
class SensoryProcessor:
    ...
    def __init__(self):
        self._events: Deque[tuple[FlowObservation, InspectionResult]] = deque(maxlen=1000)

    def register_event(self, observation: FlowObservation, inspection: InspectionResult) -> None:
    ...
    def render_snapshot(self, window: float = 60.0) -> SensorySnapshot:
        ...
        relevant = [
            (obs, ins)
            for (obs, ins) in self._events
            if now - obs.timestamp <= window
        ]
        pressure = sum(obs.payload_size for obs, _ in relevant) / max(window, 1.0)
        temperature = (
            sum(ins.anomaly_score or 0.0 for _, ins in relevant) / max(len(relevant), 1)
        )
        pain = sum(
