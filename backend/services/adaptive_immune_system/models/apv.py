...
class APVBase(BaseModel):
    ...
    priority: int = Field(..., ge=1, le=10, description="Priority level (1=critical, 10=low)")
    vulnerable_code_signature: Optional[str] = Field(None, description="Regex or AST pattern for code matching")
    vulnerable_code_type: Optional[str] = Field(None, description="Type: 'regex', 'ast-grep', 'semgrep'")
    affected_files: Optional[List[str]] = Field(default_factory=list, description="Potential affected files")
    exploitation_difficulty: Optional[str] = Field(None, description="Difficulty: 'easy', 'medium', 'hard'")
    exploitability_score: Optional[float] = Field(None, ge=0.0, le=10.0, description="Exploitability score 0-10")
    requires_human_review: bool = Field(default=False, description="Requires HITL approval")
    ...
    def validate_code_type(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None and v not in ("regex", "ast-grep", "semgrep"):
            raise ValueError("vulnerable_code_type must be 'regex', 'ast-grep', or 'semgrep'")
        return v
    ...
    def validate_difficulty(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None and v not in ("easy", "medium", "hard", "unknown"):
            raise ValueError("exploitation_difficulty must be 'easy', 'medium', 'hard', or 'unknown'")
        return v
...
class APVCreate(APVBase):
    ...
    apv_code: str = Field(..., max_length=50, description="Unique APV code (APV-YYYYMMDD-NNN)")
...
class APVUpdate(BaseModel):
    ...
    vulnerable_code_signature: Optional[str] = None
    vulnerable_code_type: Optional[str] = None
    affected_files: Optional[List[str]] = None
    exploitation_difficulty: Optional[str] = None
    exploitability_score: Optional[float] = Field(None, ge=0.0, le=10.0)
    requires_human_review: Optional[bool] = None
    dispatched_to_eureka_at: Optional[datetime] = None
    ...
    def validate_status(cls, v: Optional[str]) -> Optional[str]:
        """Validate APV status."""
        valid_statuses = ("pending", "dispatched", "confirmed", "false_positive", "remediated")
    ...
    @classmethod
    def validate_human_decision(cls, v: Optional[str]) -> Optional[str]:
        """Validate human decision."""
        if v is not None and v not in ("approved", "rejected", "deferred"):
            raise ValueError("human_decision must be 'approved', 'rejected', or 'deferred'")
        return v
...
class APVDispatchMessage(BaseModel):
    ...
    vulnerable_code_signature: Optional[str] = Field(None, description="Code pattern to search for")
    vulnerable_code_type: Optional[str] = Field(None, description="Pattern type (regex, ast-grep)")
    affected_files: List[str] = Field(default_factory=list, description="Potential affected files")

    # Exploitation metadata
    exploitation_difficulty: Optional[str] = Field(None, description="Difficulty level")
    exploitability_score: Optional[float] = Field(None, description="Exploitability score")

    # Timestamps
    dispatched_at: datetime = Field(default_factory=datetime.utcnow, description="Dispatch timestamp")
...
class APVStatusUpdate(BaseModel):
    ...
    confirmed: bool = Field(default=False, description="Vulnerability confirmed by AST scan")
    confirmation_details: Optional[dict] = Field(None, description="AST scan results")
    ...
    def validate_status(cls, v: str) -> str:
        """Validate status."""
        valid_statuses = ("received", "scanning", "confirmed", "false_positive", "error")
