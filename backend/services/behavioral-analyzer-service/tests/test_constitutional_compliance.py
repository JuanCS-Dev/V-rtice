...
def client():
    ...
    app = FastAPI()
    metrics_exporter = MetricsExporter(service_name="behavioral-analyzer-service", version="1.0.0")
    app.include_router(metrics_exporter.create_router())

    health_checker = ConstitutionalHealthCheck(service_name="behavioral-analyzer-service")
