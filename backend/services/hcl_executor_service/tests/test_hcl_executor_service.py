...
async def mock_action_executor():
    ...
    with patch("main.action_executor") as mock_ae:
        mock_ae.execute_actions = AsyncMock(
            return_value=[{"action": "scale_up", "status": "success"}, {"action": "restart", "status": "success"}]
        )
        yield mock_ae
...
async def mock_k8s_controller():
    ...
    with patch("main.k8s_controller") as mock_k8s:
        mock_k8s.get_cluster_status = AsyncMock(return_value={"nodes": 3, "pods_running": 50, "status": "healthy"})
        yield mock_k8s
...
class TestExecutePlanEndpoint:
    ...
    async def test_execute_plan_success(self, client, mock_action_executor, mock_k8s_controller):
        ...
        mock_action_executor.execute_actions.assert_called_once_with("plan-001", payload["actions"], 5)
    ...
    async def test_execute_plan_empty_actions(self, client, mock_action_executor, mock_k8s_controller):
        ...
        payload = {"plan_id": "plan-002", "actions": [], "priority": 5}
    ...
    async def test_execute_plan_many_actions(self, client, mock_action_executor, mock_k8s_controller):
        ...
        mock_action_executor.execute_actions.return_value = [
            {"action": f"action_{i}", "status": "success"} for i in range(20)
        ]

        payload = {"plan_id": "plan-large", "actions": [{"type": f"action_{i}"} for i in range(20)], "priority": 5}
    ...
    async def test_execute_plan_action_results_preserved(self, client, mock_action_executor, mock_k8s_controller):
        ...
        mock_action_executor.execute_actions.return_value = [
            {"action": "scale_up", "status": "success", "details": "Scaled to 5 replicas", "duration_ms": 250}
        ]
...
class TestEdgeCases:
    ...
    async def test_execute_plan_complex_action_structure(self, client, mock_action_executor, mock_k8s_controller):
        ...
        mock_action_executor.execute_actions.return_value = [{"action": "complex", "status": "success"}]
            ...
            "actions": [
                {
                    ...
                    "target": "service_a",
                    "replicas": 5,
                    "metadata": {"reason": "high_load", "constraints": ["max_cost_100", "min_latency_50ms"]},
                    "rollback": {"enabled": True, "threshold_error_rate": 0.05},
                }
            ],
            "priority": 8,
        }
    ...
    async def test_execute_plan_status_logic_edge_cases(self, client, mock_action_executor, mock_k8s_controller):
        ...
        mock_action_executor.execute_actions.return_value = [{"action": "a1", "status": "success"}]
        payload = create_execute_plan_request(num_actions=1)
        response = await client.post("/execute_plan", json=payload)
        assert response.json()["status"] == "completed"

        # Case 2: Single failed action
        mock_action_executor.execute_actions.return_value = [{"action": "a1", "status": "failed"}]
        response = await client.post("/execute_plan", json=payload)
