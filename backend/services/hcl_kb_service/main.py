...
async def health_check() -> Dict[str, str]:
    ...
    """
    return {"status": "healthy", "message": "HCL Knowledge Base Service is operational."}
...
async def store_hcl_data(request: StoreDataRequest) -> Dict[str, Any]:
    ...
    return {"status": "success", "message": f"{request.data_type.value} data stored.", "entry_id": len(knowledge_base[request.data_type.value]) - 1}
...
async def get_knowledge_summary() -> Dict[str, Any]:
    ...
    for data_type, entries in knowledge_base.items():
        summary[data_type] = {"count": len(entries), "last_entry": entries[-1].timestamp if entries else "N/A"}
    return summary
