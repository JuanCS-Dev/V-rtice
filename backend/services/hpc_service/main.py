...
async def train(request: TrainingRequest):
    ...
    try:
        observations = [Observation(timestamp=0, features=np.array(obs.features), source_id=obs.source_id) for obs in request.observations]
        core.learn_prior(observations)
        return {"status": "success", "message": f"Prior trained on {len(observations)} observations."}
...
async def observe(request: ObservationRequest):
    ...
    if not core.belief_state:
        raise HTTPException(status_code=400, detail="Model prior not trained. Please train first via /train.")
    
    try:
        obs = Observation(timestamp=0, features=np.array(request.features), source_id=request.source_id)
        prediction = core.predict()
        error = core.compute_prediction_error(obs, prediction)
        new_belief = core.update_beliefs(obs, error)
        return {"updated_entropy": new_belief.entropy, "prediction_error": error.magnitude}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
