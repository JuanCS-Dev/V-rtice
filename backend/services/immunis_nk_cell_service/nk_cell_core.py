    ...
    def load_whitelist(self) -> Dict[str, Any]:
        ...
        try:
            if not os.path.exists(self.whitelist_path):
                ...
                return {'status': 'default_loaded', 'entries': len(self.whitelisted_names)}
    ...
    def _load_default_whitelist(self):
        ...
        if IS_LINUX:
            ...
            self.whitelisted_names = {
                'systemd', 'sshd', 'dockerd', 'containerd', 'kubelet',
                'python3', 'bash', 'sh', 'nginx', 'apache2'
            }
        elif IS_WINDOWS:
            self.whitelisted_paths = {
...
class DLLHijackDetector:
    ...
    def _is_hijackable_dll(self, dll_name: str) -> bool:
        ...
        hijackable_dlls = {
            'dwmapi.dll', 'uxtheme.dll', 'cryptbase.dll', 'comctl32.dll',
            'version.dll', 'wtsapi32.dll', 'propsys.dll', 'explorerframe.dll'
        }
...
class NKCellCore:
    ...
    async def scan_process(
        ...
        threat_detected = (
            missing_self_result['missing_self_detected'] or
