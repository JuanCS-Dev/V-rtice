    ...
    def consume(self, tokens: int = 1) -> tuple[bool, int, int]:
        ...
        try:
            ...
            if RateLimitConfig.FAIL_OPEN:
                ...
                raise HTTPException(
                    status_code=503, detail="Rate limiting service unavailable"
                ) from e
    ...
    def __init__(
        ...
        if enabled and REDIS_AVAILABLE:
            try:
                ...
                print(
                    f"[RateLimiter] Redis connection failed: {e}. Running without rate limiting."
                )
...
def get_rate_limit_status(redis_url: str, identifier: str, path: str) -> dict:
    ...
    try:
        ...
        max_requests, window_seconds = (
            RateLimitConfig.DEFAULT_REQUESTS,
            RateLimitConfig.DEFAULT_WINDOW,
        )
