...
class RegistryClient:
    ...
    async def register(
        ...
        try:
            ...
            response = await client.post(
                f"{REGISTRY_URL}/register", json=payload, headers=cls._get_headers()
            )
            ...
            logger.warning("Service will operate WITHOUT registry (standalone mode)")
            return False

        except Exception as e:
            logger.error(f"❌ Unexpected error registering '{service_name}': {e}")
            return False
    ...
    async def heartbeat(cls, service_name: str) -> bool:
        ...
        try:
            client = cls._get_client()
            response = await client.post(
                f"{REGISTRY_URL}/heartbeat", json=payload, headers=cls._get_headers()
            )
            response.raise_for_status()
    ...
    async def deregister(cls, service_name: str) -> bool:
        ...
        try:
            ...
            response = await client.delete(
                f"{REGISTRY_URL}/deregister/{service_name}", headers=cls._get_headers()
            )
            response.raise_for_status()
    ...
    @classmethod
    async def get_service(cls, service_name: str) -> dict | None:
        """
        ...
        try:
            ...
            response = await client.get(
                f"{REGISTRY_URL}/services/{service_name}", headers=cls._get_headers()
            )
            response.raise_for_status()
            ...
            if e.response.status_code == 404:
                ...
                logger.error(
                    f"Failed to lookup '{service_name}': HTTP {e.response.status_code}"
                )
            return None
    ...
    async def list_services(cls) -> list:
        ...
        try:
            client = cls._get_client()
            response = await client.get(
                f"{REGISTRY_URL}/services", headers=cls._get_headers()
            )
            response.raise_for_status()
            return response.json()
...
async def auto_register_service(
    ...
    heartbeat_task = asyncio.create_task(RegistryClient.heartbeat_loop(service_name))
