...
class MaximusDemo:
    ...
    async def initialize_maximus(self):
        ...
        try:
            ...
            print(f"      - Neuromodulation: ✅ Active")
            print(f"      - Attention System: ✅ Active")
            print(f"      - Predictive Coding: {'✅ Available' if status['predictive_coding_status']['available'] else '⚠️  Unavailable (torch required)'}")
            print(f"      - Skill Learning: {'✅ Available' if status['skill_learning_status']['available'] else '⚠️  Unavailable (HSAS required)'}")
            print(f"      - Ethical AI: ✅ Active")
    ...
    async def process_event(self, event: Dict[str, Any], event_number: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.maximus.predictive_coding_available:
                pc_result = self.maximus.predict_with_hpc_network(
                    raw_event=event,
    ...
    def display_event_result(self, event: Dict[str, Any], result: Dict[str, Any], event_number: int):
        ...
        if result['free_energy'] > 0:
            surprise_level = "🔴 HIGH" if result['free_energy'] > 0.7 else "🟡 MEDIUM" if result['free_energy'] > 0.4 else "🟢 LOW"
            print(f"\n   🧠 Predictive Coding:")
        ...
        if result['neuromodulation_state']:
            ...
            if nm.get('attention_updated'):
                print(f"      ⚠️  Attention threshold lowered (high surprise)")
    ...
    def display_final_metrics(self):
        ...
        true_negatives = self.metrics['total_events'] - self.metrics['threats_detected'] - self.metrics['false_negatives']
        ...
        if self.metrics['prediction_errors']:
            ...
            print(f"   ⚠️  No prediction errors (Predictive Coding unavailable)")
        ...
        if self.metrics['skills_executed'] == 0:
            print(f"   ⚠️  No skills executed (HSAS service unavailable)")
        ...
        approval_rate = self.metrics['ethical_approvals'] / (self.metrics['ethical_approvals'] + self.metrics['ethical_rejections']) * 100 if (self.metrics['ethical_approvals'] + self.metrics['ethical_rejections']) > 0 else 100
