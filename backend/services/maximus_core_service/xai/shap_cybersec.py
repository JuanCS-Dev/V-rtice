...
class CyberSecSHAP(ExplainerBase):
    ...
    def _detect_model_type(self, model: Any) -> str:
        ...
        if 'xgb' in model_module or 'xgboost' in model_class:
            return 'xgboost'
        elif 'lightgbm' in model_module or 'lgbm' in model_class:
            return 'lightgbm'
        elif 'randomforest' in model_class or 'randomforestclassifier' in model_class:
    ...
    def _compute_kernel_shap(
        ...
        if self.shap_config.check_additivity:
            ...
            if abs(total - expected_total) > 0.01:
                logger.warning(
                    f"SHAP values do not add up: sum={total:.4f}, expected={expected_total:.4f}"
                )
    ...
    def _generate_summary(
        ...
        if detail_level == DetailLevel.SUMMARY:
            ...
            if positive_features:
                ...
                features_str = ", ".join([
                    f"{f.feature_name} (+{f.importance:.3f})"
                    for f in positive_features
                ])
                summary_parts.append(
                    f"Factors increasing prediction (total: +{contrib_sum:.3f}): {features_str}."
                )
            ...
            if negative_features:
                ...
                summary_parts.append(
                    f"Factors decreasing prediction (total: -{contrib_sum:.3f}): {features_str}."
                )
            ...
            summary_parts.append(f"Prediction = Base + Sum(SHAP values)")
