"""Constitutional Compliance Tests for maximus_eureka.
...
@pytest.fixture
def client():
    """Create a test client for maximus_eureka."""
    from fastapi import FastAPI
    from shared.metrics_exporter import MetricsExporter
    from shared.health_checks import ConstitutionalHealthCheck

    app = FastAPI()
    metrics_exporter = MetricsExporter(service_name="maximus_eureka", version="1.0.0")
    app.include_router(metrics_exporter.create_router())

    health_checker = ConstitutionalHealthCheck(service_name="maximus_eureka")
