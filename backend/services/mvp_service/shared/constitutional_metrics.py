...
# MVP-specific (Narrative Engine)
narrative_generations = Counter(
    "mvp_narrative_generations_total",
    "Total narrative generations",
    ["narrative_type", "quality"],
)

narrative_generation_duration = Histogram(
    "mvp_narrative_generation_duration_seconds",
    "Narrative generation duration",
    ["narrative_type"],
)

system_observations = Counter(
    "mvp_system_observations_total",
    "System observations recorded",
    ["observation_type", "severity"],
)

pii_detections = Counter(
    "mvp_pii_detections_total",
    "PII (Personally Identifiable Information) detections",
    ["pii_type", "action_taken"],
)

compliance_checks = Counter(
    "mvp_compliance_checks_total",
    "LGPD/GDPR compliance checks",
    ["check_type", "result"],
)

narrative_quality_score = Gauge(
    "mvp_narrative_quality_score", "Narrative quality score (0-1)", ["narrative_type"]
)
