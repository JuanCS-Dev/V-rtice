...
class FactCheckQueue:
        ...
        try:
            ...
            async for message in kafka_client.consume(
                ...
                try:
                    ...
                    logger.error(
                        f"Worker {worker_id} processing error: {e}",
                        exc_info=True
                    )
    ...
    async def _cache_result(self, claim_hash: str, result: Any) -> None:
        ...
        await cache_manager.set(
            category=CacheCategory.FACTCHECK,
            key=cache_key,
            value=cache_data
        )
    ...
    async def get_result(
        ...
        if wait_timeout is not None:
            ...
            async for message in kafka_client.consume(
                topic=self.TOPIC_RESULTS,
                group_id=f"result_waiter_{claim_hash}"
            ):
                data = message["value"]
