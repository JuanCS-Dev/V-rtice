        ...
        if self._producer_available:
            for attempt in range(self.max_retries + 1):
                try:
                    ...
                    logger.debug(
                        f"Published event to {topic.value} (id={event.event_id})"
                    )
                    return True
                    ...
                    if attempt < self.max_retries:
                        ...
                        logger.warning(
                            f"Publish retry {attempt + 1}/{self.max_retries + 1}: {e}"
                        )
                        await asyncio.sleep(backoff)
                    else:
                        logger.error(
                            f"Failed to publish after {self.max_retries + 1} attempts: {e}"
                        )
                        self._producer_available = False
