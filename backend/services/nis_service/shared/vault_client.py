...
class VaultClient:
    ...
    def _login_approle(self):
        ...
        try:
            ...
            logging.info(
                f"Authenticated with Vault. Token expires in {lease_duration}s"
            )
        ...
        if self.client:
            try:
                ...
                response = self.client.secrets.kv.v2.read_secret_version(
                    path=path, mount_point=VaultConfig.MOUNT_POINT
                )
    ...
    def set_secret(self, path: str, data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.client.secrets.kv.v2.create_or_update_secret(
                path=path, secret=data, mount_point=VaultConfig.MOUNT_POINT
            )
...
def get_api_key(service: str, fallback_env: str | None = None) -> str | None:
    ...
    result = client.get_secret(
        f"api-keys/{service}", "api_key", fallback_env=fallback_env
    )
...
def get_jwt_secret() -> str | None:
    """Get JWT secret key."""
    client = get_vault_client()
    result = client.get_secret("app/jwt", "secret_key", fallback_env="JWT_SECRET_KEY")
    return result if isinstance(result, str) or result is None else None
