        ...
        try:
            ...
            for capture_path in pending_captures:
                try:
                    ...
                    iocs = {
                        "ips": (
                            [parsed_data.get("attacker_ip")]
                            if parsed_data.get("attacker_ip")
                            else []
                        ),
                        "usernames": [
                            cred[0] for cred in parsed_data.get("credentials", [])
                        ],
                    ...
                    attack = AttackCreate(
                        ...
                        iocs=iocs,
                        payload=str(
                            parsed_data.get("commands", [])[:5]
                        ),  # First 5 commands
                        captured_at=(
                            parsed_data.get("timestamps", [datetime.utcnow()])[0]
                    ...
                    processed_path = capture_path.with_suffix(
                        capture_path.suffix + ".processed"
                    )
    ...
    try:
        ...
        if len(file_path.parts) >= 2:
            ...
            if "_" in parent_dir and parent_dir != "forensics":
                ...
                honeypot_id = (
                    parent_dir.split("_", 1)[1]
                    if parent_dir.count("_") >= 1
                    else parent_dir
                )
                logger.debug(
                    "honeypot_id_inferred_from_directory",
                    file=str(file_path),
                    honeypot_id=honeypot_id,
                )
                return honeypot_id
        ...
        if len(filename_parts) >= 2:
            ...
            if potential_id and potential_id != "session":
                logger.debug(
                    "honeypot_id_inferred_from_filename",
                    file=str(file_path),
                    honeypot_id=potential_id,
                )
                return potential_id
        ...
        logger.error("honeypot_id_inference_error", file=str(file_path), error=str(e))
        return "unknown"
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    try:
        ...
        configure_constitutional_logging(
            service_name="reactive_fabric_analysis",
        ...
        auto_update_sabbath_status("reactive_fabric_analysis")
        ...
        # Health
        health_checker = ConstitutionalHealthCheck(service_name="reactive_fabric_analysis")
...
from shared.health_checks import ConstitutionalHealthCheck
    ...
    uvicorn.run("main:app", host="0.0.0.0", port=8601, reload=True, log_level="info")
