...
class TTPMapper:
    ...
    TTP_PATTERNS = {
        ...
        "T1133": {  # External Remote Services
            "patterns": [r"(?i)(ssh|rdp|vnc|telnet).*login", r"(?i)remote.*access"],
            "name": "External Remote Services",
            "tactic": "Initial Access",
        },
        "T1566": {  # Phishing
            "patterns": [r"(?i)(phish|malicious.*link|credential.*harvest)"],
            "name": "Phishing",
            "tactic": "Initial Access",
        },
        # ====================================================================
        ...
        "T1053.003": {  # Scheduled Task/Job: Cron
            "patterns": [r"(?i)crontab", r"(?i)/etc/crontab", r"(?i)/var/spool/cron"],
        ...
        "T1136": {  # Create Account
            "patterns": [r"(?i)(useradd|adduser) ", r"(?i)net user.*add"],
            "name": "Create Account",
            "tactic": "Persistence",
        },
        # ====================================================================
        ...
        "T1548.003": {  # Abuse Elevation Control Mechanism: Sudo
            "patterns": [r"(?i)sudo ", r"(?i)/etc/sudoers"],
        ...
        "T1110.001": {  # Brute Force: Password Guessing
            "patterns": [r"(?i)(admin|root|user|test):.*", r"(?i)password.*guess"],
        ...
        "T1057": {  # Process Discovery
            "patterns": [r"(?i)ps ", r"(?i)top$", r"(?i)htop$", r"(?i)tasklist"],
            "name": "Process Discovery",
            "tactic": "Discovery",
        },
        "T1046": {  # Network Service Discovery
            "patterns": [r"(?i)netstat", r"(?i)ss -", r"(?i)lsof -i"],
        ...
        "T1572": {  # Protocol Tunneling
            "patterns": [r"(?i)(ssh -[DLR]|reverse.*tunnel)", r"(?i)proxychains"],
        ...
        "T1486": {  # Data Encrypted for Impact
            "patterns": [r"(?i)(ransomware|encrypt|crypt|aes)", r"(?i)openssl.*enc"],
    ...
    def _map_by_command_heuristics(self, commands: List[str]) -> Set[str]:
        ...
        if any(
            cmd in recon_commands
            for cmd in [c.split()[0] if c.split() else "" for c in commands]
        ):
