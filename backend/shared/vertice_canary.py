...
class CanaryDeploymentManager:
    ...
    def register_blue(self, service_name: str, version: str, endpoint: str, health_endpoint: str = "/health"):
        """Register blue (stable) version"""
        if service_name not in self._deployments:
            self._deployments[service_name] = {
                "stage": DeploymentStage.BLUE
            }
    ...
    def register_green(self, service_name: str, version: str, endpoint: str, health_endpoint: str = "/health"):
    ...
    def start_canary(self, service_name: str) -> bool:
        ...
        if deployment["stage"] != DeploymentStage.GREEN:
            logger.warning(f"Cannot start canary - current stage: {deployment['stage']}")
            return False

        deployment["stage"] = DeploymentStage.CANARY_5

        logger.info(f"🚀 Started CANARY (5%): {service_name} v{deployment['green'].version}")
        return True
    ...
    def promote_canary(self, service_name: str) -> bool:
        ...
        if not self._is_green_healthy(service_name):
            logger.error(f"❌ Green version unhealthy - initiating rollback for {service_name}")
        ...
        logger.info(f"📈 Promoted CANARY: {service_name} → {int(next_traffic * 100)}% traffic")
    ...
    def rollback(self, service_name: str):
        ...
        if "green" in deployment:
            ...
            logger.info(f"Keeping green v{green_version} registered but not routing traffic")
    ...
    def update_metrics(self, service_name: str, version: str, latency_ms: float, is_error: bool):
    ...
    def _is_green_healthy(self, service_name: str) -> bool:
        ...
        if green.request_count < self.min_requests_before_promote:
            logger.info(f"Green has only {green.request_count} requests (need {self.min_requests_before_promote})")
            return False

        # Check error rate
        if green.error_rate > self.max_error_rate:
            logger.error(f"Green error rate too high: {green.error_rate:.2%} > {self.max_error_rate:.2%}")
            return False
        ...
        if blue.avg_latency_ms > 0:
            ...
            if latency_ratio > self.max_latency_increase:
                logger.error(f"Green latency too high: {latency_ratio:.2f}x blue latency")
                return False

        logger.info(f"✅ Green version healthy: error_rate={green.error_rate:.2%}, latency={green.avg_latency_ms:.1f}ms")
        return True
    ...
    def _finalize_deployment(self, service_name: str):
        ...
        logger.info(f"🎉 Deployment FINALIZED: {service_name} v{new_blue.version} is now stable (blue)")
