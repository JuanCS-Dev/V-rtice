...
export const analyzeIP = async (ip) => {
  try {
    ...
    return {
      ...
      // Geolocation
      geolocation:
        data.geolocation?.status === "success"
          ? {
...
export const analyzeMyIP = async () => {
  try {
    const response = await fetch(
      `${API_ENDPOINTS.IP_INTELLIGENCE}/analyze-my-ip`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      },
 ...
 * Fixed: Updated to use /analyze-my-ip endpoint (POST) which exists in backend
 * The /my-ip (GET) endpoint was deprecated and removed
 */
...
export const checkServicesHealth = async () => {
  ...
  try {
    ...
    const ipResp = await fetch(
      `${API_ENDPOINTS.IP_INTELLIGENCE.replace("/api/ip", "")}/`,
      {
        method: "GET",
  ...
  try {
    // Threat Intel
    const threatResp = await fetch(
      `${API_ENDPOINTS.THREAT_INTEL.replace("/api/threat-intel", "")}/`,
      {
        method: "GET",
        signal: AbortSignal.timeout(3000),
      },
