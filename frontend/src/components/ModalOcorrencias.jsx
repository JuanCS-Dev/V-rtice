        ...
        <main className="flex-1 p-6 overflow-y-auto">
          <div className="space-y-6">
            {ocorrencias.length > 0 ? (
              ...
              <div key={ocorrencia.id} className="border border-green-400/30 rounded-lg bg-black/30 p-4">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <span className="font-bold text-green-400">{ocorrencia.tipo}</span>
                    <span className="ml-4 text-green-400/60 text-sm font-mono">ID: {ocorrencia.id}</span>
                  </div>
                  <span className={`px-2 py-1 text-xs rounded-full ${
                    ocorrencia.status === 'Em Investigação' ? 'bg-yellow-500/20 text-yellow-400' : 'bg-gray-500/20 text-gray-400'
                  }`}>{ocorrencia.status}</span>
                </div>
              ...
              <p className="text-center text-green-400/70">Nenhuma ocorrência registrada para este veículo.</p>
            )}
          </div>
        </main>
      </div>
