...
const SystemSelfCheck = () => {
  ...
  const processSecurityResults = async (rawData) => {
    ...
    if (rawData.step_0) {
      ...
      categories.fileIntegrity = {
        score: fileData.error
          ? 50
          : fileData.modified_files?.length > 0
            ? 70
            : 95,
        ...
        details: fileData.error
          ...
          : [
              ...
              fileData.suid_files
                ? `${fileData.suid_files.length} arquivos SUID encontrados`
                : "Nenhum arquivo SUID suspeito",
            ],
      };
    ...
    if (rawData.step_1) {
      ...
      categories.processAnalysis = {
        score: processData.error
          ? 60
          : processData.suspicious_processes?.length > 0
            ? 75
            : 90,
        issues: processData.suspicious_processes?.length || 0,
        details: processData.error
          ? ["Erro na análise"]
          : [
    ...
    if (rawData.step_2) {
      ...
      categories.networkSecurity = {
        ...
        issues:
          portData.open_ports?.filter(
            (p) =>
              !["22", "80", "443", "5173", "8000", "8001"].includes(p.port),
          ).length || 0,
        details: portData.error
          ? ["Erro na análise"]
          : [
    ...
    if (rawData.step_4) {
      ...
      categories.certificates = {
        score: certData.error
          ? 70
          : certData.expired_certs?.length > 0
            ? 75
            : 95,
        issues: certData.expired_certs?.length || 0,
        details: certData.error
          ? ["Erro na verificação"]
          : [
              `${certData.valid_certs || 0} certificados válidos`,
              certData.expired_certs?.length > 0
                ? "Certificados expirados encontrados"
  ...
  const runSecurityScan = async () => {
    ...
    const scanSteps = [
      {
        name: "Verificando integridade de arquivos críticos...",
        func: runFileIntegrityCheck,
      },
      ...
      {
        name: "Validando certificados SSL/TLS...",
        func: runCertificateValidation,
      },
      {
        name: "Verificando configurações de segurança...",
        func: () => checkSecurityConfig(),
      },
      {
        name: "Analisando logs de segurança...",
        func: () => analyzeSecurityLogs(),
      },
      {
        name: "Compilando relatório final...",
    ...
    try {
      ...
      setScanResults({
        ...
        recommendations: [
          "Verificar conectividade com serviços de backend",
    ...
    <div className="space-y-6">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-yellow-400/70 mt-1">
              Verificação abrangente com análise real de segurança e integridade
            </p>
          </div>
            ...
            {lastScanTime && (
              <div className="text-right text-sm">
                ...
                <div className="text-yellow-400">
                  {formatDateTime(lastScanTime, "N/A")}
                </div>
              </div>
            )}
        ...
        <>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-yellow-400/70">
                  Baseado em verificações reais do sistema
                </p>
                {scanResults.scanDuration && (
                  <p className="text-yellow-400/50 text-sm">
                    Duração do scan: {scanResults.scanDuration}
                  </p>
                )}
              </div>
              <div className="text-center">
                <div
                  className={`text-6xl font-bold ${getScoreColor(scanResults.overallScore)}`}
                >
          ...
          {Object.keys(scanResults.categories).length > 0 && (
            <div className="grid grid-cols-2 gap-6">
              {Object.entries(scanResults.categories).map(
                ([category, data]) => (
                  <div
                    key={category}
                    className="border border-yellow-400/30 rounded-lg bg-black/20 p-4"
                  >
                    ...
                    <div className="space-y-1">
                      {data.details.map((detail, index) => (
                        <div
                          key={index}
                          className="text-xs text-yellow-400/70 flex items-center"
                        >
            ...
            <div className="border border-yellow-400/30 rounded-lg bg-black/20 p-6">
              ...
              <div className="space-y-3">
                {scanResults.threats.map((threat, index) => (
                  <div
                    key={index}
                    className={`p-3 rounded border-l-4 ${getThreatColor(threat.severity)}`}
                  >
          ...
          <div className="border border-yellow-400/30 rounded-lg bg-black/20 p-6">
            ...
            <div className="space-y-2">
              {scanResults.recommendations.map((rec, index) => (
                <div
                  key={index}
                  className="bg-black/40 border border-yellow-400/20 rounded p-3 flex items-center"
                >
      ...
      {!scanResults && !isScanning && (
        <div className="border border-yellow-400/30 rounded-lg bg-black/20 p-8 text-center">
          ...
          <h3 className="text-yellow-400 text-xl mb-2">
            REAL-TIME SECURITY CHECK
          </h3>
          <p className="text-yellow-400/70 mb-6">
            Execute verificações reais de segurança usando módulos cyber
            integrados
          </p>
          ...
          <div className="grid grid-cols-3 gap-6 mt-8">
            <div className="bg-black/40 border border-yellow-400/20 rounded p-4">
              ...
              <p className="text-yellow-400/70 text-xs mt-1">
                Portas, processos e arquivos reais do sistema
              </p>
            </div>
            ...
            <div className="bg-black/40 border border-yellow-400/20 rounded p-4">
              ...
              <p className="text-yellow-400/70 text-xs mt-1">
                Integração com módulos Batman do Cerrado
              </p>
            </div>
            ...
            <div className="bg-black/40 border border-yellow-400/20 rounded p-4">
              ...
              <p className="text-yellow-400/70 text-xs mt-1">
                Dados reais para auditoria SSP-GO
              </p>
            </div>
          </div>
        </div>
