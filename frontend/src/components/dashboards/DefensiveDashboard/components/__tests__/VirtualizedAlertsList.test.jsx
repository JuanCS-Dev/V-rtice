...
describe('VirtualizedAlertsList', () => {
  ...
  it('handles large datasets efficiently (1000+ alerts)', () => {
    ...
    const { container } = render(<VirtualizedAlertsList alerts={largeDataset} />);
  ...
  it('displays statistics bar with correct severity counts', () => {
    const alerts = [
      { id: 'alert-1', severity: 'critical', type: 'TEST', message: 'msg1', timestamp: new Date().toISOString() },
      { id: 'alert-2', severity: 'critical', type: 'TEST', message: 'msg2', timestamp: new Date().toISOString() },
      { id: 'alert-3', severity: 'high', type: 'TEST', message: 'msg3', timestamp: new Date().toISOString() },
      { id: 'alert-4', severity: 'medium', type: 'TEST', message: 'msg4', timestamp: new Date().toISOString() },
      { id: 'alert-5', severity: 'medium', type: 'TEST', message: 'msg5', timestamp: new Date().toISOString() },
      { id: 'alert-6', severity: 'low', type: 'TEST', message: 'msg6', timestamp: new Date().toISOString() },
    ];
  ...
  it('applies correct CSS classes for severity levels', () => {
    ...
    const criticalAlert = container.querySelector('[class*="severity-critical"]');
  ...
  it('renders container with stats', () => {
    const { container } = render(
      <VirtualizedAlertsList alerts={mockAlerts} />
    );
