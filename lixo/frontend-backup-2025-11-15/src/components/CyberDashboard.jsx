    ...
    const alertTimer = setInterval(() => {
      if (Math.random() > 0.9) {
        const alertTypes = [
          {
            type: "MALWARE",
            message: "Domínio malicioso detectado",
            severity: "critical",
          },
          {
            type: "PHISHING",
            message: "Tentativa de phishing identificada",
            severity: "high",
          },
          {
            type: "BOTNET",
            message: "Atividade de botnet detectada",
            severity: "high",
          },
          { type: "SCAN", message: "Port scan detectado", severity: "medium" },
          {
            type: "INTEL",
            message: "Nova inteligência de ameaça",
            severity: "info",
          },
        ];
  ...
  return (
    <div className={styles.overviewContainer}>
      <div className={styles.overviewCard}>
        <h2 className={styles.overviewTitle}>CENTRO DE OPERAÇÕES CYBER</h2>
        ...
        <div className={styles.metricsGrid}>
          ...
          <div className={`${styles.metricCard} ${styles.warning}`}>
            <div className={`${styles.metricValue} ${styles.warning}`}>
              {threatData.activeDomains}
            </div>
            <div className={styles.metricLabel}>DOMÍNIOS SUSPEITOS</div>
          </div>
          <div className={`${styles.metricCard} ${styles.alert}`}>
            <div className={`${styles.metricValue} ${styles.alert}`}>
              {threatData.suspiciousIPs}
            </div>
            <div className={styles.metricLabel}>IPS MALICIOSOS</div>
          </div>
          <div className={`${styles.metricCard} ${styles.info}`}>
            <div className={`${styles.metricValue} ${styles.info}`}>
              {threatData.networkAlerts}
            </div>
        ...
        <div className={styles.modulesSection}>
          ...
          <div className={styles.modulesGrid}>
            ...
            <div className={`${styles.moduleCard} ${styles.offensive}`}>
              <div className={styles.moduleHeader}>
                <span className={`${styles.moduleName} ${styles.offensive}`}>
                  Vulnerability Scanner ⚠️
                </span>
                <div
                  className={`${styles.statusDot} ${styles.offensive}`}
                ></div>
              </div>
              <div className={`${styles.moduleInfo} ${styles.offensive}`}>
                Ferramenta ofensiva ativa
              </div>
            </div>
            <div className={`${styles.moduleCard} ${styles.offensive}`}>
              <div className={styles.moduleHeader}>
