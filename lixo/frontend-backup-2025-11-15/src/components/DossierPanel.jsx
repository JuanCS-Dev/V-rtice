  ...
  return (
    <div className="w-96 border-r border-green-400/30 bg-black/20 backdrop-blur-sm overflow-y-auto p-4 flex flex-col flex-shrink-0">
      <div className="border border-green-400/50 rounded-lg bg-green-400/5 p-4 mb-4 flex-grow">
        <h3 className="text-green-400 font-bold text-xl mb-4 tracking-wider">
          DOSSIÊ VEICULAR
        </h3>
        <div className="mb-4 flex items-center space-x-2">
          <div
            className={`inline-block px-3 py-1 rounded-full text-xs font-bold ${
              dossierData.riskLevel === "HIGH"
                ? "bg-red-500/20 text-red-400 border border-red-400/50"
                : dossierData.riskLevel === "MEDIUM"
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <span className="text-green-400/70 text-xs block">PLACA</span>
              <span className="text-green-400 font-bold text-lg">
                {dossierData.placa}
              </span>
            </div>
            <div>
              <span className="text-green-400/70 text-xs block">SITUAÇÃO</span>
          ...
          <div>
            <span className="text-green-400/70 text-xs block">VEÍCULO</span>
            <span className="text-green-400 font-bold">
              {dossierData.marca} {dossierData.modelo}
            </span>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <span className="text-green-400/70 text-xs block">ANO</span>
              <span className="text-green-400">{dossierData.ano}</span>
            </div>
            <div>
              <span className="text-green-400/70 text-xs block">COR</span>
              <span className="text-green-400">{dossierData.cor}</span>
            </div>
          </div>
          <div>
            <span className="text-green-400/70 text-xs block">CHASSI</span>
            <span className="text-green-400 font-mono">
              {dossierData.chassi}
            </span>
          </div>
          <div>
            <span className="text-green-400/70 text-xs block">LOCALIZAÇÃO</span>
            <span className="text-green-400">
              {dossierData.municipio} - {dossierData.uf}
            </span>
          </div>
        </div>
      </div>
