...
export const ThreatGlobeWithOnion = ({ realThreats = [] }) => {
  ...
  useEffect(() => {
    if (!mapInstanceRef.current || realThreats.length === 0 || isTracing)
      return;
  ...
  const startOnionTrace = useCallback(
    async (targetIp) => {
  ...
  const addNodeMarker = useCallback(
    (node) => {
    ...
    `;
      ...
      const pulseInterval = setInterval(() => {
        ...
        setTimeout(
          () => circle.setRadius(node.type === "destination" ? 15 : 10),
          300,
        );
        pulseCount++;
      }, 800);
        ...
        {isTracing && (
          <div className="onion-status">
            ...
            <div className="onion-progress-bar">
              <div
                className="onion-progress-fill"
                style={{ width: `${traceProgress}%` }}
              ></div>
            </div>
            <div className="onion-message">{traceStatus}</div>
            <div className="onion-hop">
              HOP {currentHop}/{currentRoute.length - 1}
            </div>
          </div>
        )}
