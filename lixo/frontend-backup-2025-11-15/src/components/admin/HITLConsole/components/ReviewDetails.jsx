...
const ReviewDetails = ({ review, loading, apvSelected }) => {
  ...
  const tabs = [
    ...
    {
      id: "wargame",
      label: t("hitl.details.wargame_tab", "Wargame"),
      icon: "🎮",
    },
    {
      id: "validation",
      label: t("hitl.details.validation_tab", "Validation"),
      icon: "✅",
    },
  ];
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        ...
        <span
          className={`${styles.severityBadge} ${styles[`severity${review.severity}`]}`}
        >
  ...
  return (
    <div className={styles.section}>
      <h3 className={styles.sectionTitle}>
        {t("hitl.details.cve_info", "CVE Information")}
      </h3>
      <div className={styles.field}>
        <span className={styles.label}>
          {t("hitl.details.cve_id", "CVE ID")}:
        </span>
        <span className={styles.value}>{review.cve_id}</span>
      </div>
      <div className={styles.field}>
        <span className={styles.label}>
          {t("hitl.details.title", "Title")}:
        </span>
        <span className={styles.value}>{review.cve_title}</span>
      </div>
      <div className={styles.field}>
        <span className={styles.label}>
          {t("hitl.details.description", "Description")}:
        </span>
        <p className={styles.description}>{review.cve_description}</p>
      </div>
      <div className={styles.field}>
        <span className={styles.label}>
          {t("hitl.details.cvss", "CVSS Score")}:
        </span>
        <span className={styles.value}>{review.cvss_score}</span>
      </div>
      <div className={styles.field}>
        <span className={styles.label}>
          {t("hitl.details.package", "Package")}:
        </span>
        <span className={styles.value}>
          {review.package_name} {review.package_version} →{" "}
          {review.fixed_version}
        </span>
      </div>
      <div className={styles.field}>
        <span className={styles.label}>
          {t("hitl.details.confidence", "Confirmation")}:
        </span>
        <div className={styles.confidenceBar}>
          <div
            className={styles.confidenceFill}
            style={{ width: `${review.confirmation_confidence * 100}%` }}
          />
          <span className={styles.confidenceValue}>
            {(review.confirmation_confidence * 100).toFixed(1)}%
          </span>
        </div>
      </div>
    </div>
...
const PatchTab = ({ review }) => {
  ...
  return (
    <div className={styles.section}>
      <h3 className={styles.sectionTitle}>
        {t("hitl.details.patch_info", "Patch Information")}
      </h3>
      <div className={styles.field}>
        <span className={styles.label}>
          {t("hitl.details.strategy", "Strategy")}:
        </span>
        <span className={styles.value}>{review.patch_strategy}</span>
      </div>
      <div className={styles.field}>
        <span className={styles.label}>
          {t("hitl.details.patch_desc", "Description")}:
        </span>
      ...
      {review.pr_url && (
        <div className={styles.field}>
          <span className={styles.label}>
            {t("hitl.details.pr", "Pull Request")}:
          </span>
          <a
            href={review.pr_url}
            target="_blank"
...
const WargameTab = ({ review }) => {
  ...
  return (
    <div className={styles.section}>
      <h3 className={styles.sectionTitle}>
        {t("hitl.details.wargame_results", "Wargame Results")}
      </h3>
      <div className={styles.field}>
        <span className={styles.label}>
          {t("hitl.details.verdict", "Verdict")}:
        </span>
        <span
          className={`${styles.verdict} ${styles[`verdict${review.wargame_verdict}`]}`}
        >
      ...
      <div className={styles.field}>
        <span className={styles.label}>
          {t("hitl.details.wargame_confidence", "Confidence")}:
        </span>
        <span className={styles.value}>
          {(review.wargame_confidence * 100).toFixed(1)}%
        </span>
      </div>
      {review.wargame_run_url && (
        <div className={styles.field}>
          <span className={styles.label}>
            {t("hitl.details.workflow", "Workflow")}:
          </span>
          <a
            href={review.wargame_run_url}
            target="_blank"
            rel="noopener noreferrer"
            className={styles.link}
          >
...
const ValidationTab = ({ review }) => {
  ...
  return (
    <div className={styles.section}>
      <h3 className={styles.sectionTitle}>
        {t("hitl.details.validation_results", "Validation Results")}
      </h3>
      <div className={styles.field}>
        <span className={styles.label}>
          {t("hitl.details.status", "Status")}:
        </span>
        <span
          className={`${styles.value} ${review.validation_passed ? styles.success : styles.error}`}
        >
      ...
      <div className={styles.field}>
        <span className={styles.label}>
          {t("hitl.details.validation_confidence", "Confidence")}:
        </span>
        <span className={styles.value}>
          {(review.validation_confidence * 100).toFixed(1)}%
        </span>
      </div>
