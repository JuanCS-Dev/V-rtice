...
export const BAS = () => {
  ...
  const detectionRate =
    totalSimulations > 0
      ? ((detectedSimulations / totalSimulations) * 100).toFixed(1)
      : 0;
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-red-400/60 text-sm mt-1">
              MITRE ATT&CK Framework | Purple Team Validation | Coverage
              Analysis | Port 8036
            </p>
          </div>
        ...
        <div className="flex justify-between items-center text-xs text-red-400/60">
          <div className="flex gap-4">
            <span role="status" aria-live="polite">
              STATUS: {isSimulating ? "🟣 SIMULATING" : "🟢 READY"}
            </span>
            <span>FRAMEWORK: MITRE ATT&CK v14.0</span>
            <span>MODE: {simulationConfig.platform.toUpperCase()}</span>
          </div>
          <div>BREACH & ATTACK SIMULATION v3.0 | MAXIMUS AI POWERED</div>
        </div>
      </footer>
