  ...
  const handleAnalyzeIP = useCallback(async (ipToAnalyze) => {
    ...
    try {
      ...
      const formattedResult = {
        ...
        reputation: {
          score: threatIntel.success
            ? threatIntel.threatScore
            : ipAnalysis.reputation?.score || 0,
          categories: threatIntel.success
            ? threatIntel.categories
            : ipAnalysis.reputation?.categories || [],
          last_seen: threatIntel.success
            ? threatIntel.lastSeen
            : ipAnalysis.reputation?.lastSeen || "N/A",
        },
        ...
        threat_intel: threatIntel.success
          ? {
              ...
              sources: Object.keys(threatIntel.sources || {}).filter(
                (k) => threatIntel.sources[k]?.available,
              ),
      ...
      setSearchHistory((prev) =>
        [ipToAnalyze, ...prev.filter((ip) => ip !== ipToAnalyze)].slice(0, 10),
      );
    } catch (error) {
