...
export const useCommandBus = () => {
  ...
  const sendCommand = useCallback(
    async (commandType, targetAgents, parameters = {}) => {
      setLoading(true);
      setError(null);
      ...
      try {
        const payload = {
          ...
          target_agents: Array.isArray(targetAgents)
            ? targetAgents
            : [targetAgents],
          parameters,
          operator_id:
            localStorage.getItem("operatorId") || "sovereign-operator",
  ...
  const muteAgent = useCallback(
    (agentId, duration = 300) => {
      return sendCommand(COMMAND_TYPES.MUTE, agentId, {
  ...
  const isolateAgent = useCallback(
    (agentId, reason = "Sovereign decision") => {
