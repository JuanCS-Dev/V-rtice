...
describe('VirtualizedExecutionsList', () => {
  ...
  it('handles large datasets efficiently (1000+ items)', () => {
    ...
    const { container } = render(<VirtualizedExecutionsList executions={largeDataset} />);

    // Should render virtualized list (only renders visible items)
    expect(screen.getByTestId('virtualized-list')).toBeInTheDocument();

    // Should NOT render all 1000 items (performance check)
    const renderedItems = container.querySelectorAll('[class*="executionCard"]');
    expect(renderedItems.length).toBeLessThan(100); // Only renders visible window
  });

  it('displays statistics bar with correct counts', () => {
    const executions = [
      { id: 'exec-1', status: 'running', type: 'SCAN', target: 'test1', timestamp: new Date().toISOString() },
      { id: 'exec-2', status: 'running', type: 'SCAN', target: 'test2', timestamp: new Date().toISOString() },
      { id: 'exec-3', status: 'completed', type: 'SCAN', target: 'test3', timestamp: new Date().toISOString() },
      { id: 'exec-4', status: 'failed', type: 'SCAN', target: 'test4', timestamp: new Date().toISOString() },
    ];
