...
export const AdaptiveImmunityPanel = ({ aiStatus, setAiStatus }) => {
  ...
  logger.debug("🧬 AdaptiveImmunityPanel rendering...", {
    timeRange,
    activeTab,
  });
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(
        `/oraculo/wargaming/ml/stats?time_range=${timeRange}`,
      );
  ...
  const { data: confidenceData, isLoading: confLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/oraculo/wargaming/ml/confidence-distribution?time_range=${timeRange}`,
      );
      if (!response.ok)
        throw new Error("Failed to fetch confidence distribution");
      return response.json();
    },
  ...
  const { data: recentPredictions, isLoading: predLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/oraculo/wargaming/ml/recent-predictions?limit=20&time_range=${timeRange}`,
      );
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(
        `/oraculo/wargaming/ml/accuracy?time_range=${timeRange}`,
      );
      if (!response.ok) {
      ...
      <div className="flex items-center justify-center h-full bg-gray-900">
        <div className="text-center">
          ...
          <div className="text-red-400 text-xl">
            Loading Adaptive Immunity metrics...
          </div>
          <div className="text-gray-500 text-sm mt-2">
            Initializing ML monitoring
          </div>
        </div>
      </div>
    );
  ...
  const validationMethodData = [
    {
      name: "ML Only",
      value: mlStats?.ml_only_validations || 0,
      color: "#10b981",
    },
    {
      name: "Wargaming Fallback",
      value: mlStats?.wargaming_fallbacks || 0,
      color: "#f59e0b",
    },
  ];

  const confidenceBins =
    confidenceData?.bins?.map((bin, i) => ({
      range: `${(bin * 100).toFixed(0)}-${((confidenceData.bins[i + 1] || 1) * 100).toFixed(0)}%`,
      count: confidenceData.counts[i] || 0,
    })) || [];

  const speedupFactor =
    mlStats?.avg_wargaming_time_ms && mlStats?.avg_ml_time_ms
        ...
        <>
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <Card className="p-5 bg-gradient-to-br from-gray-800 to-gray-900 border-2 border-red-500/50 hover:border-red-500 transition-all">
              ...
              <div className="text-xs text-gray-500">Threshold: 80%</div>
            </Card>
            ...
            <Card className="p-5 bg-gradient-to-br from-gray-800 to-gray-900 border-2 border-yellow-500/50 hover:border-yellow-500 transition-all">
              ...
              <div className="text-xs text-gray-500">vs Full Wargaming</div>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Card className="p-5 bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700">
              ...
              <div className="text-center text-sm text-gray-400 mt-2">
                Threshold:{" "}
                {((confidenceData?.threshold || 0.8) * 100).toFixed(0)}%
              </div>
            </Card>
          </div>
                ...
                <div>
                  <div className="flex justify-between mb-2">
                    ...
                    <span className="text-yellow-400 font-mono">
                      {((mlStats?.avg_wargaming_time_ms || 0) / 1000).toFixed(
                        1,
                      )}
                      s
                    </span>
              ...
              {accuracyLoading ? (
                ...
                <div className="flex items-center justify-center h-full">
                  <div className="text-center">
                    ...
                    <div className="text-gray-500 text-sm mt-2">
                      Ensure PostgreSQL is running and A/B tests have been
                      executed
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-center h-full">
                  <div className="text-center">
                    ...
                    <div className="text-gray-400 text-lg">
                      A/B Testing Not Yet Active
                    </div>
                    <div className="text-gray-500 text-sm mt-2">
                      Run some ML validations to trigger A/B testing
                    </div>
                    <div className="mt-4 text-xs text-gray-600">
                      A/B testing will compare ML predictions vs wargaming
                      ground truth
                    </div>
                  </div>
                </div>
          ...
          <Card className="p-4 bg-gradient-to-r from-red-900/30 to-green-900/30 border-2 border-red-500/50">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="text-green-400 animate-pulse text-2xl">
                  ● ACTIVE
                </div>
        ...
        <div className="space-y-6">
          <Card className="p-8 bg-gradient-to-br from-orange-900/20 to-red-900/20 border-2 border-orange-500 text-center">
            ...
            <p className="text-gray-400 mb-4">
              Coming in Phase 5.8: Real-time comparison of ML vs Wargaming
              accuracy
            </p>
            <p className="text-sm text-gray-500">
              This tab will display A/B test results, confusion matrices, and
