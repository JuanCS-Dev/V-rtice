...
export const ConsciousnessPanel = ({ aiStatus, setAiStatus }) => {
  ...
  useEffect(() => {
    setStreamStatus({
      connected: !!isConnected && connectionType !== "idle",
      type: connectionType,
    });
  ...
  const handleAdjustArousal = async () => {
    const result = await adjustArousal(
      arousalDelta,
      arousalDuration,
      "dashboard_manual",
    );
      ...
      <div className="consciousness-header">
        <div className="header-left">
          <h2 className="consciousness-title">
            ...
            <span
              className={`status-dot ${streamStatus.connected ? "connected" : "disconnected"}`}
            ></span>
          </h2>
          <p className="consciousness-subtitle">
            ...
            {lastUpdate && (
              <span className="last-update">
                {" "}
                • Updated {formatEventTime(lastUpdate.toISOString())}
              </span>
            )}
          </p>
        </div>
        ...
        <div className="header-right">
          <div className={`arousal-badge ${arousalInfo.borderClass}`}>
            ...
            <div className="arousal-info">
              ...
              <div className="arousal-value">
                {(arousalState?.arousal || 0).toFixed(2)}
              </div>
            </div>
          </div>
          ...
          {consciousnessState && (
            <div className="system-health">
              <span
                className={`health-indicator ${consciousnessState.system_health?.toLowerCase()}`}
              >
            ...
            <div className="arousal-gauge-container">
              ...
              <div className="gauge-center-label">
                ...
                <div className="gauge-label">
                  {formatArousalLevel(arousalState?.level).label}
                </div>
              </div>
            </div>
            <div className="arousal-breakdown">
              <div className="breakdown-item">
                ...
                <span className="breakdown-value">
                  {(arousalState?.baseline || 0).toFixed(2)}
                </span>
              </div>
              <div className="breakdown-item">
                <span className="breakdown-label">Needs:</span>
                <span className="breakdown-value">
                  +{(arousalState?.need_contribution || 0).toFixed(2)}
                </span>
              </div>
              <div className="breakdown-item">
                <span className="breakdown-label">Stress:</span>
                <span className="breakdown-value">
                  +{(arousalState?.stress_contribution || 0).toFixed(2)}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="metrics-grid">
              ...
              <div className="metric-item">
                ...
                <div className="metric-value">
                  {tigMetrics.eci?.toFixed(3) || "N/A"}
                </div>
              </div>
              <div className="metric-item">
                <div className="metric-label">Clustering</div>
                <div className="metric-value">
                  {tigMetrics.clustering?.toFixed(3) || "N/A"}
                </div>
              </div>
              <div className="metric-item">
                <div className="metric-label">Path Length</div>
                <div className="metric-value">
                  {tigMetrics.path_length?.toFixed(2) || "N/A"}
                </div>
              </div>
              <div className="metric-item">
                <div className="metric-label">Connectivity</div>
                <div className="metric-value">
                  {tigMetrics.algebraic_connectivity?.toFixed(3) || "N/A"}
                </div>
              </div>
              <div className="metric-item">
                <div className="metric-label">Density</div>
                <div className="metric-value">
                  {tigMetrics.density?.toFixed(3) || "N/A"}
                </div>
              </div>
            </div>
            ...
            <div className="stats-list">
              ...
              <div className="stat-row">
                ...
                <span className="stat-value">
                  {esgtEvents.length > 0 &&
                  esgtEvents.some(
                    (e) => e.coherence !== null && e.coherence !== undefined,
                  )
                    ? esgtEvents
                        .filter(
                          (e) =>
                            e.coherence !== null && e.coherence !== undefined,
                        )
                        .reduce((sum, e) => sum + e.coherence, 0) /
                      esgtEvents
                        .filter(
                          (e) =>
                            e.coherence !== null && e.coherence !== undefined,
              ...
              <div className="stat-row">
                ...
                <span
                  className={`stat-value ${streamStatus.connected ? "success" : "error"}`}
                >
                  {streamStatus.connected
                    ? `🟢 Connected (${streamStatus.type.toUpperCase()})`
                    : "🔴 Disconnected"}
                </span>
              </div>
            </div>
  ...
  const renderEventCard = (event, isCompact = false) => {
    return (
      <div
        className={`esgt-event-card ${event.success ? "success" : "failed"} ${isCompact ? "compact" : ""}`}
      >
        <div className="event-header">
          <span
            className={`event-status ${event.success ? "success" : "failed"}`}
          >
          ...
          <div className="event-metrics">
            <div className="event-metric">
              ...
              <span className="metric-value">
                {event.coherence !== null && event.coherence !== undefined
                  ? event.coherence.toFixed(3)
                  : "N/A"}
              </span>
            </div>
            <div className="event-metric">
            ...
            <div className="event-salience">
              <div className="salience-bar">
                ...
                <div
                  ...
                  style={{ width: `${(event.salience.novelty || 0) * 100}%` }}
                ></div>
                <span className="salience-value">
                  {(event.salience.novelty || 0).toFixed(2)}
                </span>
              </div>
              <div className="salience-bar">
                ...
                <div
                  className="salience-fill"
                  style={{ width: `${(event.salience.relevance || 0) * 100}%` }}
                ></div>
                <span className="salience-value">
                  {(event.salience.relevance || 0).toFixed(2)}
                </span>
              </div>
              <div className="salience-bar">
                ...
                <div
                  className="salience-fill"
                  style={{ width: `${(event.salience.urgency || 0) * 100}%` }}
                ></div>
                <span className="salience-value">
                  {(event.salience.urgency || 0).toFixed(2)}
                </span>
              </div>
            </div>
          )}
        ...
        <div className="control-grid">
          ...
          <div className="control-card esgt-trigger-card">
            ...
            <p className="control-description">
              Trigger a conscious access event by specifying salience
              components.
            </p>
            ...
            {triggerResult && !triggerResult.loading && (
              <div
                className={`trigger-result ${triggerResult.success ? "success" : "error"}`}
              >
                {triggerResult.success ? (
                  <>
                    <div>
                      ✅ Ignition{" "}
                      {triggerResult.success ? "succeeded" : "failed"}
                    </div>
                    <div className="result-details">
                      Event ID: {triggerResult.event_id} | Coherence:{" "}
                      {triggerResult.coherence?.toFixed(3) || "N/A"} | Duration:{" "}
                      {triggerResult.duration_ms?.toFixed(1) || "N/A"} ms
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <div className="result-details">
                      Reason:{" "}
                      {triggerResult.reason || triggerResult.error || "Unknown"}
                    </div>
                  </>
                )}
              </div>
          ...
          <div className="control-card arousal-adjust-card">
            ...
            <div className="control-inputs">
              ...
              <div className="slider-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setArousalDuration(parseFloat(e.target.value))
                  }
            ...
            <div className="arousal-preview">
              ...
              <div className="preview-value">
                {Math.max(
                  0,
                  Math.min(1, (arousalState?.arousal || 0) + arousalDelta),
                ).toFixed(2)}
              </div>
            </div>
          </div>
          ...
          <div className="control-card info-card">
            ...
            <div className="info-list">
              <div className="info-item">
                ...
                <span
                  className={`info-value ${consciousnessState?.esgt_active ? "active" : "inactive"}`}
                >
              ...
              <div className="info-item">
                ...
                <span className="info-value">
                  {consciousnessState?.recent_events_count || 0}
                </span>
              </div>
              <div className="info-item">
                <span className="info-label">System Health:</span>
                <span
                  className={`info-value ${consciousnessState?.system_health?.toLowerCase()}`}
                >
              ...
              <div className="info-item">
                ...
                <span
                  className={`info-value ${streamStatus.connected ? "success" : "error"}`}
                >
                  {streamStatus.connected
                    ? `🟢 Connected (${streamStatus.type.toUpperCase()})`
                    : "🔴 Disconnected"}
              ...
              <div className="info-item">
                ...
                <span className="info-value">
                  {lastUpdate
                    ? formatEventTime(lastUpdate.toISOString())
                    : "Never"}
                </span>
              </div>
            </div>
  ...
  return (
    <div className="consciousness-panel">
      ...
      <div className="consciousness-content">
        ...
        {selectedView === "safety" && (
          <SafetyMonitorWidget
            systemHealth={consciousnessState?.system_health}
          />
        )}
      </div>
    </div>
