...
export const PendingPatchCard = ({ patch, isSelected, onSelect }) => {
  ...
  return (
    <Card
      onClick={onSelect}
      className={`p-5 cursor-pointer transition-all duration-300 transform hover:scale-105 hover:shadow-2xl ${getSeverityColor(
        patch.severity,
      )} ${
        isSelected
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <Badge
          className={`${getSeverityBadgeColor(patch.severity)} px-3 py-1 text-xs font-bold`}
        >
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          ...
          <span
            className={`text-sm font-bold ${getConfidenceColor(patch.ml_confidence)}`}
          >
        ...
        <div className="w-full bg-gray-800 rounded-full h-2 overflow-hidden">
          <div
            className={`h-full transition-all ${
              patch.ml_confidence >= 0.95
                ? "bg-gradient-to-r from-green-500 to-green-400"
                : patch.ml_confidence >= 0.8
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-700">
        <div className="flex items-center gap-2 text-xs text-gray-400">
          ...
          <span>
            Waiting{" "}
            <span className="font-bold text-gray-300">{ageDisplay}</span>
          </span>
        </div>
