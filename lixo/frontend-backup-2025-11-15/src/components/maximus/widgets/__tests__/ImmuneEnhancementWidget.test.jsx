...
vi.mock("../../../../api/maximusAI", () => ({
  suppressFalsePositives: vi.fn(() =>
    Promise.resolve({ total_alerts: 0, suppressed_count: 0 }),
  ),
  consolidateMemory: vi.fn(() =>
    Promise.resolve({ patterns_count: 0, ltm_entries_created: 0 }),
  ),
...
describe("ImmuneEnhancementWidget Component", () => {
  beforeEach(() => {
    ...
    suppressFalsePositives.mockResolvedValue({
      total_alerts: 0,
      suppressed_count: 0,
    });
    consolidateMemory.mockResolvedValue({
      patterns_count: 0,
      ltm_entries_created: 0,
    });
  ...
  it("should switch to consolidation tab", async () => {
    ...
    expect(
      screen.getByText(/Trigger memory consolidation cycle/),
    ).toBeInTheDocument();
  ...
  it("should switch to LTM Query tab", async () => {
    ...
    expect(
      screen.getByText(/Query long-term immunological memory/),
    ).toBeInTheDocument();
    expect(
  ...
  describe("FP Suppression Tab", () => {
    it("should show error when alerts input is empty", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Please provide alerts in JSON format/),
        ).toBeInTheDocument();
      });
    });
    ...
    it("should show error for non-array input", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Alerts must be a non-empty array/),
        ).toBeInTheDocument();
      });
    });
  ...
  describe("LTM Query Tab", () => {
    ...
    it("should call queryLongTermMemory with correct parameters", async () => {
      ...
      const mockResult = {
        memories: [
          {
            pattern_type: "Attack Chain",
            importance: 0.92,
            description: "Advanced APT pattern",
          },
        ],
      };
        ...
        memories: [
          {
            ...
            importance: 0.95,
            description: "WannaCry-like behavior",
          },
          {
