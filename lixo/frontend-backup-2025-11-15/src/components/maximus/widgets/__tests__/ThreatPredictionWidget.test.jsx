...
describe("ThreatPredictionWidget Component", () => {
  ...
  it("should display hunting recommendations", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("Monitor for unusual PowerShell activity"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Check for lateral movement indicators"),
      ).toBeInTheDocument();
    });
  });
  ...
  it('should show "no threats predicted" when empty', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("No vulnerabilities forecasted"),
      ).toBeInTheDocument();
  ...
  it("should show placeholder before first prediction", () => {
    ...
    expect(
      screen.getByText(/Configure parameters and run prediction/),
    ).toBeInTheDocument();
  });
  ...
  it("should hide placeholder after prediction", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText(/Configure parameters and run prediction/),
  ...
  it("should handle prediction with custom parameters", async () => {
    ...
    await user.selectOptions(
      screen.getByLabelText("Time Horizon (hours)"),
      "72",
    );
