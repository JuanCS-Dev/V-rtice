      ...
      <div className="border border-red-400/50 rounded-lg bg-red-400/5 p-6">
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <p className="text-red-300 text-xs leading-relaxed">
                Esta ferramenta é destinada exclusivamente para propósitos de
                segurança defensiva e inteligência de ameaças. O uso para
                atividades ilegais é estritamente proibido e pode resultar em
                consequências legais. Todos os acessos são registrados e
                monitorados.
              </p>
            </div>
          </div>
        ...
        {results && !monitoring && (
          <div className="bg-black/50 border border-red-400/30 rounded-lg p-6">
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              ...
              <div className="bg-black/40 border border-yellow-400/30 rounded-lg p-3 text-center">
                <div className="text-yellow-400 text-2xl font-bold">
                  {results.threat_keywords_matched || 0}
                </div>
                <div className="text-yellow-400/70 text-xs">Threats</div>
              </div>
              <div
                className={`rounded-lg p-3 text-center border ${getThreatColor(results.threat_score || 0)}`}
              >
                <div className="text-2xl font-bold">
                  {results.threat_score?.toFixed(1) || 0}
                </div>
            ...
            {results.threat_score > 0 && (
              <div
                className={`mb-6 p-4 rounded-lg border ${getThreatColor(results.threat_score)}`}
              >
                <h4 className="font-bold mb-2">🚨 THREAT ASSESSMENT</h4>
                <p className="text-sm opacity-90">
                  {results.threat_score >= 75 &&
                    "CRITICAL: Ameaças ativas detectadas! Investigação imediata necessária."}
                  {results.threat_score >= 50 &&
                    results.threat_score < 75 &&
                    "HIGH: Múltiplas ameaças potenciais encontradas."}
                  {results.threat_score >= 25 &&
                    results.threat_score < 50 &&
                    "MEDIUM: Algumas menções encontradas. Monitorar."}
                  {results.threat_score < 25 &&
                    "LOW: Poucas ou nenhuma ameaça detectada."}
                </p>
              </div>
            )}
            ...
            {results.findings && results.findings.length > 0 ? (
                      ...
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-red-400/60">
                            Source: {finding.source} | Discovered:{" "}
              ...
              <div className="text-center p-8 text-red-400/60">
                ...
                <p className="text-xs mt-2">
                  O termo buscado não foi encontrado em marketplaces ou fóruns
                  monitorados.
                </p>
              </div>
            )}
        ...
        <div className="mt-4 bg-black/40 border border-red-400/20 rounded p-3 text-center">
          <p className="text-red-400/60 text-xs">
            🔒 Este serviço utiliza apenas fontes públicas da clearnet
            (Ahmia.fi, Onionland). Nenhuma conexão direta à rede Tor é
            estabelecida.
          </p>
        </div>
      </div>
