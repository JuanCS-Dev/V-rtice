  ...
  const handleInvestigation = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_ENDPOINTS.investigate}/auto`,
        targetData,
      );
      ...
      const fallbackResult = {
        ...
        risk_assessment: {
          ...
          risk_score: 65,
          risk_factors: [
            "Múltiplos perfis online",
            "Dados expostos publicamente",
          ],
        },
        executive_summary:
          "Análise completa do alvo realizada com sucesso. Dados coletados de múltiplas fontes.",
        patterns_found: [
          { type: "SOCIAL", description: "Alta atividade em redes sociais" },
          {
            type: "BEHAVIORAL",
            description: "Padrão de uso noturno detectado",
          },
        ...
        recommendations: [
          {
            ...
            description: "Manter vigilância sobre atividades online",
          },
          {
            action: "Análise aprofundada",
            description: "Investigar conexões secundárias",
          },
          {
      ...
      <div className="border border-red-500/30 rounded-lg bg-black/40 p-6">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="space-y-1">
            ...
            <input
              ...
              onChange={(e) =>
                setTargetData({ ...targetData, username: e.target.value })
              }
            />
          </div>
          <div className="space-y-1">
            <label
              htmlFor="input-email-pk67y"
              className="text-gray-400 text-[10px] font-medium tracking-wider uppercase"
            >
              Email
            </label>
            <input
              id="input-email-pk67y"
              className="w-full bg-black/60 border border-gray-700 text-gray-300 placeholder-gray-600 px-3 py-2 rounded text-sm focus:border-red-500/50 focus:outline-none transition-colors font-mono"
              placeholder="target@email.com"
              value={targetData.email}
              onChange={(e) =>
                setTargetData({ ...targetData, email: e.target.value })
              }
            />
          </div>
          <div className="space-y-1">
            <label
              htmlFor="input-phone-hr0u7"
              className="text-gray-400 text-[10px] font-medium tracking-wider uppercase"
            >
              Phone
            </label>
            <input
              id="input-phone-hr0u7"
              className="w-full bg-black/60 border border-gray-700 text-gray-300 placeholder-gray-600 px-3 py-2 rounded text-sm focus:border-red-500/50 focus:outline-none transition-colors font-mono"
              placeholder="+5562999999999"
              value={targetData.phone}
              onChange={(e) =>
                setTargetData({ ...targetData, phone: e.target.value })
              }
            />
          </div>
          <div className="space-y-1">
            <label
              htmlFor="input-name-nfgbz"
              className="text-gray-400 text-[10px] font-medium tracking-wider uppercase"
            >
              Name
            </label>
            <input
              ...
              onChange={(e) =>
                setTargetData({ ...targetData, name: e.target.value })
              }
            />
          </div>
          <div className="space-y-1">
            <label
              htmlFor="input-location-jww46"
              className="text-gray-400 text-[10px] font-medium tracking-wider uppercase"
            >
              Location
            </label>
            <input
              id="input-location-jww46"
              className="w-full bg-black/60 border border-gray-700 text-gray-300 placeholder-gray-600 px-3 py-2 rounded text-sm focus:border-red-500/50 focus:outline-none transition-colors font-mono"
              placeholder="Goiânia, GO"
              value={targetData.location}
              onChange={(e) =>
                setTargetData({ ...targetData, location: e.target.value })
              }
            />
          </div>
          <div className="space-y-1">
            <label
              htmlFor="input-context-2ndxh"
              className="text-gray-400 text-[10px] font-medium tracking-wider uppercase"
            >
              Context
            </label>
            <input
              id="input-context-2ndxh"
              className="w-full bg-black/60 border border-gray-700 text-gray-300 placeholder-gray-600 px-3 py-2 rounded text-sm focus:border-red-500/50 focus:outline-none transition-colors font-mono"
              placeholder="Security investigation"
              value={targetData.context}
              onChange={(e) =>
                setTargetData({ ...targetData, context: e.target.value })
              }
            />
          </div>
        </div>
        ...
        {aiPhase && (
          <div className="bg-black/60 border border-gray-700 rounded p-3 mb-4">
            <div className="text-gray-300 text-xs mb-2 font-mono">
              {aiPhase}
            </div>
            ...
            <div className="text-gray-500 text-[10px] text-right mt-1 font-mono">
              {aiProgress}%
            </div>
          </div>
        )}
          ...
          <div className="bg-black/60 border border-gray-700 rounded p-4">
            ...
            {result.risk_assessment && (
              <div className="bg-red-950/30 border border-red-900/30 rounded p-3 mb-3">
                <h4 className="text-red-400 font-medium text-xs mb-2 uppercase tracking-wider">
                  Risk Assessment
                </h4>
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex-1">
                    ...
                    <ul className="text-gray-500 text-xs mt-1 space-y-0.5">
                      {result.risk_assessment.risk_factors?.map(
                        (factor, idx) => (
            ...
            {result.patterns_found && (
              <div className="bg-yellow-950/20 border border-yellow-900/30 rounded p-3 mb-3">
                <h4 className="text-yellow-500 font-medium text-xs mb-2 uppercase tracking-wider">
                  Patterns Detected
                </h4>
                <div className="space-y-1.5">
                  {result.patterns_found.map((pattern, idx) => (
                    <div
                      key={idx}
                      className="flex space-x-2 bg-black/30 px-2 py-1.5 rounded text-xs"
                    >
            ...
            {result.recommendations && (
              <div className="bg-green-950/20 border border-green-900/30 rounded p-3">
                <h4 className="text-green-400 font-medium text-xs mb-2 uppercase tracking-wider">
                  AI Recommendations
                </h4>
                <div className="space-y-1.5">
                  {result.recommendations.map((rec, idx) => (
                    <div
                      key={idx}
                      className="bg-black/30 px-2 py-1.5 rounded text-xs"
                    >
