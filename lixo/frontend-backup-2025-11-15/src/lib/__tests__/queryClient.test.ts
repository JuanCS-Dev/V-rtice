...
describe('queryClient', () => {
  describe('createQueryClient', () => {
    ...
    it('should retry server errors', () => {
      const queryClient = createQueryClient();
      const retry = queryClient.getDefaultOptions().mutations?.retry as Function;
    ...
    it('should use exponential backoff for retry delay', () => {
      const queryClient = createQueryClient();
      const retryDelay = queryClient.getDefaultOptions().mutations?.retryDelay as Function;
  ...
  describe('queryKeys', () => {
    ...
    it('should have vulnerability keys', () => {
      ...
      expect(queryKeys.vulnerability.lists()).toEqual(['vulnerabilities', 'list']);
      expect(queryKeys.vulnerability.list({ severity: 'high' })).toEqual([
        'vulnerabilities',
        'list',
        { filters: { severity: 'high' } },
      ]);
      expect(queryKeys.vulnerability.details()).toEqual(['vulnerabilities', 'detail']);
    ...
    it('should have metrics keys', () => {
      ...
      expect(queryKeys.metrics.timeRange('7d')).toEqual(['metrics', 'timeRange', '7d']);
    });
  ...
  describe('mutationKeys', () => {
    it('should have scan mutation keys', () => {
      ...
      expect(mutationKeys.scan.delete('456')).toEqual(['scan', 'delete', '456']);
    });
  ...
  describe('invalidation helpers', () => {
    ...
    it('invalidateVulnerabilities should invalidate vulnerability queries', async () => {
      ...
      queryClient.setQueryData(queryKeys.vulnerability.all, { vulnerabilities: [] });
