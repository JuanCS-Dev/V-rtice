...
app = typer.Typer(
    name="hunt",
    help="🔎 Threat hunting operations",
...
@app.command()
def search(
    query: Annotated[str, typer.Argument(help="IOC query to hunt for (IP, domain, hash, etc.)")],
    ioc_type: Annotated[Optional[str], typer.Option("--type", "-t", help="IOC type: ip, domain, hash, email, url")] = None,
    ...
    try:
        with spinner_task("Connecting to Threat Intelligence service..."):
            if not connector.health_check():
                print_error("Threat Intelligence service is not available")
                return
        ...
        with spinner_task(f"Hunting for IOC: {query}..."):
            result = connector.search_threat(query)
        ...
        if json_output:
            ...
            if 'threat_data' in result and result['threat_data']:
                ...
                console.print(f"[cyan]Reputation:[/cyan] {threat.get('reputation', 'Unknown')}")

                if 'risk_score' in threat:
                    risk = threat['risk_score']
                    risk_color = 'red' if risk >= 70 else 'yellow' if risk >= 40 else 'green'
                    console.print(f"[cyan]Risk Score:[/cyan] [{risk_color}]{risk}/100[/{risk_color}]\n")
        ...
        print_error(f"Threat hunt failed: {str(e)}")
    finally:
        connector.close()
...
@app.command()
def timeline(
    incident_id: Annotated[str, typer.Argument(help="Incident ID to generate timeline for")],
    last: Annotated[str, typer.Option("--last", help="Timeframe (e.g., 1h, 24h, 7d)")] = "24h",
    json_output: Annotated[bool, typer.Option("--json", "-j", help="Output as JSON")] = False,
    verbose: Annotated[bool, typer.Option("--verbose", "-v", help="Verbose output")] = False
):
    """
    ...
    try:
        ...
        with spinner_task(f"Building threat timeline for {incident_id}..."):
            # Get threat timeline
            result = connector.get_threat_timeline(incident_id, timeframe=last)
        ...
        print_error(f"Timeline generation failed: {str(e)}")
    finally:
        connector.close()
...
def pivot(
    ...
    try:
        ...
        with spinner_task(f"Pivoting on IOC: {ioc} (depth={depth})..."):
            result = connector.pivot_analysis(ioc, depth=depth)
        ...
        print_error(f"Pivot analysis failed: {str(e)}")
    finally:
        connector.close()
...
def correlate(
    ...
    try:
        ...
        with spinner_task(f"Correlating {ioc1} with {ioc2}..."):
            result = connector.correlate_threats(ioc1, ioc2)
        ...
        print_error(f"Correlation analysis failed: {str(e)}")
    finally:
        connector.close()
