...
async def _execute_maximus_command(
    ...
    try:
        with Live(
            Spinner(
                "dots",
                text=f"[bold bright_magenta]{spinner_text}[/bold bright_magenta]",
            ),
            console=console,
            transient=True,
            ...
            if not await connector.health_check():
                print_error(
                    f"O serviço '{connector.service_name}' está offline ou inacessível em {connector.base_url}"
                )
        ...
        if response_data:
            ...
            print_error(
                "Não foi possível obter uma resposta da Maximus AI. A resposta foi vazia."
            )
            raise typer.Exit(code=1)
...
def ask(
    question: Annotated[
        str,
        typer.Argument(
            help="A pergunta ou prompt em linguagem natural para a Maximus AI."
        ),
    ],
    json_output: Annotated[
        bool, typer.Option("--json", "-j", help="Exibir a saída bruta em formato JSON.")
    ] = False,
):
    """
    ...
    connector = AIAgentConnector()
    asyncio.run(
        _execute_maximus_command(
            connector.query,
            question,
            json_output,
            spinner_text="Maximus AI pensando...",
        )
    )
...
def analyze(
    context: Annotated[
        str,
        typer.Argument(
            help="O contexto (ex: logs, código, relatório) para a Maximus AI analisar."
        ),
    ],
    json_output: Annotated[
        bool, typer.Option("--json", "-j", help="Exibir a saída bruta em formato JSON.")
    ...
    asyncio.run(
        _execute_maximus_command(
            ...
            context,
            json_output,
            spinner_text="Maximus AI analisando contexto...",
        )
    )
...
def investigate(
    incident: Annotated[
        str,
        typer.Argument(help="Detalhes de um incidente para a Maximus AI investigar."),
    ],
    json_output: Annotated[
        bool, typer.Option("--json", "-j", help="Exibir a saída bruta em formato JSON.")
    ] = False,
...
def chat():
    ...
    console.print(
        "[yellow]Funcionalidade 'chat' em desenvolvimento. Consulte o blueprint para detalhes.[/yellow]"
    )
...
def oraculo():
    ...
    console.print(
        "[yellow]Funcionalidade 'oraculo' em desenvolvimento. Consulte o blueprint para detalhes.[/yellow]"
    )
...
def eureka(
    code_path: Annotated[
        str, typer.Argument(help="Caminho para o código a ser analisado.")
    ],
):
    ...
    console.print(
        f"[yellow]Funcionalidade 'eureka' para analisar '{code_path}' em desenvolvimento. Consulte o blueprint para detalhes.[/yellow]"
    )
